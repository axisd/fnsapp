...
{
	_neg    = v._neg;
	_val.assign( v._val.begin(), v._val.end() );
}
...
large::large(long long i)
{
	...
	else
	{
		...
		if( _neg )
			i    = -i;
		_val.push_back(dword_lo(i));
		_val.push_back(dword_hi(i));
	}
	normalize();
}

large::large(unsigned long long i)
{
	_neg    = false;
	if( i != 0 )
	{
...
large large::operator-( const large& v ) const
{
	...
	for( size_t i = 0; i < r._val.size(); i++ )
	{
		if( i < v._val.size() )
		{
			dword_t    d    = (dword_t) r._val[i] - v._val[i] + k + 0xFFFFFFFF;
			w    = dword_lo(d);
			k    = dword_hi(d);
		}
		else
		{
			if( k == 1 )    break;
			dword_t    d    = (dword_t) r._val[i] + k + 0xFFFFFFFF;
			w    = dword_lo(d);
			k    = dword_hi(d);
		}
		r._val[i]    = w;
...
long long large::tolonglong()
{
	...
	{
		r = ((dword_t)_val[0]& 0x7FFFFFFFFFFFFFFF);
	}
	else
	{
		r = ((((dword_t)_val[0]& 0x7FFFFFFFFFFFFFFF)) | ((dword_t)_val[1]<< 32));
	}
	return *this < 0 ? - r : r;
}
